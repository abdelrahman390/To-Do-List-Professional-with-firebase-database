...
let tasksData,
	...
	sliderDeleteHistoryButton = document.querySelector(
		"section main .slider .bottom .delete-history"
	),
	...
	deletedTaskContainer = document.querySelector(
		"section main .deleted-task-main .deleted-tasks-container"
	),
	deletedTaskExitButton = document.querySelector(
		"section main .deleted-task-main .exit"
	),
	tasksContainer = document.querySelector("section main .tasks-container"),
	deleteTaskButton = document.querySelector(
		"section main .slider .clear-tasks"
	),
	noButton = document.querySelector(
...
function handleAccountSection() {
	let accountSection = document.querySelector(
			"section header .max-width .account"
		),
		tasksContainer = document.querySelector("section main .tasks-container "),
...
function loginAndRegister() {
	...
	let hidePassword = document.querySelectorAll(
		".before_login > .container .box form .cont .container img"
	);
	...
	/************** register **************/
	let registerCard = document.querySelector(
		".before_login .container .register"
	);
	let registerForm = document.querySelector(".register form");
	let userName = registerCard.querySelector(
		" .before_login .container .box .cont  .name"
	);
	let password = registerCard.querySelector(
		" .before_login .container .box .cont  .password"
	);
	let registerButton = document.querySelector(
		".before_login .container .box  .register_button"
	);
	let confirmPassword = document.querySelector(
		".before_login .container .box .cont  .confirm_password"
	);
	let registrationPasswordAlarm = document.querySelector(
		".before_login .container .box.register .cont .alarm.password"
	);
	let registrationUserNameAlarm = document.querySelector(
		".before_login .container .box.register .cont .alarm.userName"
	);
	...
	let userNameInput = document.querySelector(
		".before_login .container .box.logIn form .cont .name"
	);
	let passwordInput = document.querySelector(
		"section main .before_login > .container .box form .cont .container input"
	);
	let loginAlarm = document.querySelector(
		".before_login .container .box.logIn .cont .alarm"
	);
	let loginButton = document.querySelector(
		".before_login .container .box.logIn  .login_button"
	);

	loginButton.addEventListener("click", function () {
		for (const key in all_users) {
			if (
				userNameInput.value == all_users[key].userName &&
				passwordInput.value == all_users[key].password
			) {
...
function handleMainSectionHight() {
	if (window.innerWidth <= 600) {
		let viewportHeight = window.innerHeight,
			...
			TasksWantedHight =
				viewportHeight -
				(header.offsetHeight + footer.offsetHeight + slider.offsetHeight + 5);
		const root = document.documentElement;
		root.style.setProperty("--tasks-hight", `${TasksWantedHight}px`); // Change to tomato color

		mainWantedHight =
			viewportHeight - (header.offsetHeight + footer.offsetHeight);
		...
		TasksWantedHight =
			viewportHeight - (header.offsetHeight + footer.offsetHeight + 85);
		const root = document.documentElement;
		root.style.setProperty("--tasks-hight", `${TasksWantedHight}px`); // Change to tomato color
...
function checkIfAllTasksAreDone(task) {
	...
	try {
		...
		if (allTasksDone) {
			// Set opacity to 0.7
			task[0].parentElement.parentElement.setAttribute(
				"data-status",
				"complete"
			);
			...
			if (
				task[0].parentElement.parentElement.querySelector(".complete-date") !=
				null
			) {
				task[0].parentElement.parentElement
					.querySelector(".complete-date")
					.remove();
			}
		}
	} catch (error) {
	...
	try {
		...
		function createTaskCard() {
			...
			Object.keys(task["data"]).forEach((key, index) => {
				...
				task["data"][key]["status"] == "complete"
					? (taskContentCheckBox.checked = true)
					: (taskContentCheckBox.checked = false);
...
function tasksMain() {
	...
	function createTask() {
		...
		addTaskButton.addEventListener("click", function () {
			...
			addTaskButton.addEventListener("click", () => {
				const taskCount =
					containerDiv.getElementsByClassName("task").length + 1;
					...
					element.addEventListener("click", function () {
						...
						addTaskButton.parentElement
							...
							.forEach((element, index) => {
								element
									.querySelector(".cont")
									.querySelector(".num").innerHTML = `${index + 1} -`;
							});
					});
				});
			}
	...
	function completeTasks() {
		let completeTasksInput = document.querySelectorAll(
			".task-card .container .task  input"
		);
			...
			element.addEventListener("click", function () {
				...
				if (element.checked) {
					...
					if (
						...
						element.parentElement.parentElement.children[1].querySelector(
							".complete_date"
						).innerHTML = "Complete date: Not completed";
					}
				}
				...
				let taskData = JSON.parse(sessionStorage.getItem("allTasks"))[id][
					"taskData"
				];
				...
				element.parentElement.parentElement.parentElement
					.querySelectorAll(".task")
					.forEach((element, index) => {
						let addedDate = element
							.querySelector(".added_date")
							.innerHTML.split(":")
							.slice(1)
							.toLocaleString();
						let num = element.querySelector(".cont").firstChild.innerText;
						let taskContent =
							element.querySelector(".cont").lastChild.innerText;
						let status = element.getAttribute("data-status");

						if (element.querySelector("input").checked) {
							const completedDate = element
								.querySelector(".bottomTaskCont")
								.querySelector(".complete_date")
								.innerHTML.split(":")
								.slice(1)
								.toLocaleString();
...
function addSettingToTasksBox() {
	let tasksCard = document.querySelectorAll(
	...
	deleteButton = document.querySelectorAll(
		".task-card .trash  .settingBox .delete"
	);
}

function tasksSetting() {
	settingButton.forEach((settingButton, index) => {
		let didntDelete = settingButton.parentElement.querySelector(
				".task-card .trash  .settingBox .boxesCont .no"
			),
			confirmDelete = settingButton.parentElement.querySelector(
				".task-card .trash  .settingBox .boxesCont .delete"
			),
			DeleteButton = settingButton.parentElement.querySelector(
				".task-card .trash  .settingBox .delete"
			),
			taskContainer =
				settingButton.parentElement.parentElement.querySelector(".container"),
			editButton = settingButton.parentElement.parentElement.querySelectorAll(
				".trash  .settingBox .edit"
			),
			TaskCard = settingButton.parentElement.parentElement,
			TitleDiv =
				settingButton.parentElement.parentElement.querySelector(".title"),
			containerDiv =
				settingButton.parentElement.parentElement.querySelectorAll(
					".container .task"
				),
			dateDiv =
				settingButton.parentElement.parentElement.querySelector(".date "),
			deleteImg =
				settingButton.parentElement.parentElement.querySelector(".deleteImg"),
			EditedTaskDate = null;
		...
		settingButton.addEventListener("click", function () {
			...
			function deleteTask() {
				DeleteButton.addEventListener("click", function () {
					settingButton.parentElement
						.querySelector(".task-card .trash  .settingBox ")
						.querySelector(".deleteBox")
				...
				confirmDelete.addEventListener("click", function () {
					let elementId =
						settingButton.parentElement.parentElement.getAttribute("data-id");
			...
			function editTask() {
				...
				editButton.forEach((element) => {
					element.addEventListener("click", function () {
						...
						settingButton.parentElement.parentElement
						...
						if (
							settingButton.parentElement.parentElement.getAttribute(
								"data-status"
							) == "complete"
						) {
							settingButton.parentElement.parentElement.classList.add(
								"on-edit"
							);
						}
						...
						function editTaskHandle() {
							...
							containerDiv.forEach((element) => {
								let taskClass =
									element.querySelector(".cont").children[1].className;
								let taskValue =
									element.querySelector(".cont").children[1].innerText;
								let taskInput = document.createElement("input");
								taskInput.classList.add(`${taskClass}`);
								taskInput.value = taskValue;
								element
									.querySelector(".cont")
									.children[1].replaceWith(taskInput);
								...
								element
									.querySelector(".cont")
									.parentElement.lastChild.replaceWith(deleteTaskImg);
								...
								deleteTaskImg.addEventListener("click", function () {
									...
									let container =
										...
										allTasksHeight =
											container.parentElement.querySelectorAll(".task"),
										containerHeight = 0;
						...
						function SaveEditedTask() {
							let DoneButtonB =
								settingButton.parentElement.parentElement.querySelector(
									".DoneButton"
								);
							...
							DoneButtonB.addEventListener("click", function () {
								...
								settingButton.parentElement.parentElement
									...
									.forEach((element, index) => {
										...
										let alreadyExistTasks = JSON.parse(
											sessionStorage.getItem("allTasks")
										)[id]["taskData"]["data"];
										let alreadyExistTasksLength =
											Object.keys(alreadyExistTasks).length;
										let addedDate;
										...
										if (alreadyExistTasks[index] === undefined) {
											...
											if (
												alreadyExistTasks[index]["complete_date"] == undefined
											) {
												taskData = {
													num: num,
													content: taskContent,
												...
												let completeDate =
													alreadyExistTasks[index]["complete_date"];
												taskData = {
													num: num,
													content: taskContent,
													status: `${status}`,
								...
								let editedTask = {
									...
									date: `${
										JSON.parse(sessionStorage.getItem("allTasks"))[id][
											"taskData"
										]["date"]
									}`,
									data: allTasksData,
			...
			if (numberOfBoxes > 3) {
				if (index + 2 <= numberOfBoxes) {
					if (index == 0) {
						...
						element.style.cssText = `left: ${
							index * (taskWidth + gap) +
							(freeSpace / (numberOfBoxes - 1)) * index
						}px`;
					}
				} else {
					let topElement =
						allTasksCard[index + 1 - numberOfBoxes].clientHeight + 10;
					let styles = window.getComputedStyle(
						allTasksCard[index + 1 - numberOfBoxes]
					);
					let topElementTopPosition = styles
						.getPropertyValue("top")
						.split("px")[0];
					element.style.cssText = `top: ${
						+topElementTopPosition + +topElement + gap
					}px; left: ${styles.getPropertyValue("left")}`;
				}
				...
				if (index + 2 <= numberOfBoxes) {
					if (index == 0) {
						...
						element.style.cssText = `left: ${
							index * (taskWidth + gap) + (freeSpace / 3) * 2
						}px`;
					}
				} else {
					let topElement =
				...
				if (index == 0) {
					...
					if (index == 1) {
						...
						let topElementTopPosition = styles
							...
							.split("px")[0];
						element.style.cssText = `top: ${
							+topElementTopPosition + topElement + gap + 20
						}px; left: ${styles.getPropertyValue("left")}`;
					} else {
						let topElement =
							allTasksCard[index + 1 - numberOfBoxes].clientHeight + 10;
...
function handleEveryNestedTaskData() {
	let test = document.querySelectorAll(
		".tasks-container .task-card .container .task"
	);
		...
		if (!allNestedTasksThatAddedToItEvent.includes(element)) {
			...
			element.addEventListener("click", function () {
				...
				if (element.querySelector(".bottomTaskCont") !== null) {
					let taskHeight =
						element.querySelector(".bottomTaskCont").scrollHeight;

					if (element.classList.contains("open")) {
						element.querySelector(".bottomTaskCont").style.height = `${
							taskHeight - 10
						}px`;
					} else {
...
function handleOpenAndCloseTasks() {
	let allTasksTitles = document.querySelectorAll(
