...
interface Module {
    // addTask: (message: string, receiver: string) => void;
    // addUser: (userName: string, password: string | number) => void;
...
async function addTask(id: number, taskData: {}) {
...
async function addUser(id: number, taskData: {}) {
    console.log(id, taskData)
    try {
        await setDoc(doc(db, "tasks", id), {
            id: id ,
...
async function deleteTaskFromDatabase(taskId: number) {
    try {
        console.log(taskId)
...
function addTasksFromDataBase(id: number, taskData: {}, oldTasksAfterEdit?: {}) {
    let taskCars ;
